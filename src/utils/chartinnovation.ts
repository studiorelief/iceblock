...
function chartInnovation() {
  ...
  async function drawChartInnovation() {
    ...
    if (chartInnovation) {
      // Si chartInnovation existe déjà, le détruire avant d'en créer un nouveau
      chartInnovation.destroy();
    }
    ...
    // Filtrer les données pour le portfolio '🧊 Innovation'
    data = data.filter((row) => row[1] === '🧊 Innovation');
  ...
  async function ChartJsCMS() {
    ...
    let portfolioFilterValue = 'Innovation Portfolio'; // Initial portfolio filter value
    ...
    drawChartInnovation(); // Draw the initial chart
  }

  ChartJsCMS(); // Call the function to set up the chart
}

export { chartInnovation };
